...
import os
# Forcer le rendu logiciel Mesa sous WSL (nécessaire pour un contexte OpenGL valide)
...
# Patch PyOpenGL context detection pour WSL (SDL2 utilise EGL, mais PyOpenGL cherche GLX)
_original_GetCurrentContext = _gl_platform.GetCurrentContext
def _sdl_GetCurrentContext():
    """Utilise SDL2 pour détecter le contexte OpenGL courant (compatible WSL/EGL)."""
    ctx = sdl2.SDL_GL_GetCurrentContext()
    if ctx:
        return ctx
    return _original_GetCurrentContext()
_gl_platform.GetCurrentContext = _sdl_GetCurrentContext
...
class Visualizer3D:
    ...
    def _sdl_error_message(self):
        """
        Retourne le message d'erreur SDL courant sous forme de chaîne lisible.
        """
    ...
    def _ensure_gl_context_current(self):
        """
        Vérifie et active le contexte OpenGL pour le thread courant.
        """
        if not self.window:
            raise RuntimeError("Fenêtre SDL non initialisée pour le contexte OpenGL.")
        if not self.gl_context:
            raise RuntimeError("Contexte OpenGL non initialisé.")

        if sdl2.SDL_GL_MakeCurrent(self.window, self.gl_context) != 0:
            raise RuntimeError(
                f"Erreur SDL_GL_MakeCurrent: {self._sdl_error_message()}"
            )

        if not sdl2.SDL_GL_GetCurrentContext():
            raise RuntimeError(
                "Contexte OpenGL invalide: aucun contexte courant après SDL_GL_MakeCurrent."
            )
    ...
    def _init_sdl(self):
        ...
        if sdl2.SDL_Init(sdl2.SDL_INIT_VIDEO) != 0:
            raise RuntimeError(f"Erreur SDL_Init: {self._sdl_error_message()}")
        ...
        if not self.window:
            raise RuntimeError(f"Erreur création fenêtre: {self._sdl_error_message()}")
        
        # Création du contexte OpenGL
        self.gl_context = sdl2.SDL_GL_CreateContext(self.window)
        if not self.gl_context:
            raise RuntimeError(
                f"Erreur création contexte GL: {self._sdl_error_message()}"
            )

        self._ensure_gl_context_current()
